...
export default async function CarrierPage({ params }: Params) {
  ...
  const t = await getTranslations("carrierDetail");
      ...
      <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
        ...
        <header className="mt-6 flex flex-col items-start gap-6 sm:flex-row sm:items-center">
          <span className="flex h-24 w-36 items-center justify-center rounded-3xl border border-gray-100 bg-gray-50 p-4">
            <Image
              src={carrier.logoSrc}
              alt={t("logoAlt", { carrier: carrierName })}
              width={132}
              height={72}
          ...
          <div>
            ...
            <h1 className="mt-2 text-4xl font-bold text-navy sm:text-5xl">
              {t("heading", { carrier: carrierName })}
            </h1>
            <p className="mt-4 max-w-2xl text-lg text-gray-600">
              {t("intro", { carrier: carrierName })}
            </p>
          </div>
        </header>
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2">
          <section className="rounded-3xl border border-gray-100 bg-white p-6 shadow-[0_18px_46px_-36px_rgba(0,32,92,0.25)]">
            <h2 className="text-lg font-bold text-navy">{t("howWeUse", { carrier: carrierName })}</h2>
            <p className="mt-3 text-sm text-gray-600">{t("howWeUseBody")}</p>
          </section>
          <section className="rounded-3xl border border-gray-100 bg-white p-6 shadow-[0_18px_46px_-36px_rgba(0,32,92,0.25)]">
            <h2 className="text-lg font-bold text-navy">{t("talkToPerson")}</h2>
            <p className="mt-3 text-sm text-gray-600">
              {t.rich("talkToPersonBody", {
                phone: () => (
            ...
            <div className="mt-5 flex flex-wrap gap-3">
              <Link
                ...
                href={`/quote?carrier=${slug}` as any}
              ...
              >
                {t("compareQuote", { carrier: carrierName })}
              </Link>
              <a
        ...
        <section className="mt-14 rounded-3xl bg-navy px-6 py-10 text-white sm:px-10">
          <h2 className="text-2xl font-bold">
            {t("whyIndependent", { carrier: carrierName })}
          </h2>
          <ul className="mt-5 grid gap-3 text-sm text-white/85 sm:grid-cols-2">
            <li>{t("benefitCompare", { carrier: carrierName })}</li>
            <li>{t("benefitAdvocacy", { carrier: carrierName })}</li>
            <li>{t("benefitReview")}</li>
            <li>{t("benefitNoPressure", { carrier: carrierName })}</li>
          </ul>
        </section>

        <section className="mt-14">
          <h2 className="text-xl font-bold text-navy">{t("otherCarriers")}</h2>
              ...
              .map((related) => {
                ...
                return (
                  <Link
                    key={related.name}
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    href={`/carriers/${relatedSlug}` as any}
